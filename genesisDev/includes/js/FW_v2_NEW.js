	...
	var FW_V2_IMG_PATH = "/genesisDev/imagens";
	var FW_V2_null     = "]|.NULL.|[";
	...
	function FW_V2_componenteHtml(pId) 	 		 {
		...
		switch(FW_V2_tipoCom) {
			case "TEXT_AREA"://__[
				...
				FW_V2_DIV_CONTENT_ID = $("#FW_V2_COMP_" + pId).dialog({
					autoOpen: false, 
						 modal: true,
					position: {my: "center", at: "top+150", of: window},
					 ...
					 buttons: [{
								 id: "bttAnexoSalvar",
							 text: "Aplicar",
							click: function() {FW_V2_componenteTexto(pId); $(this).dialog("close");}
						 
						}, {
								 id: "bttAnexoSair",
							 text: "Sair",
							click: function() {FW_V2_retornarTexto(pId, FW_V2_tipoCom); $(this).dialog("close");}
			...
			case "SELECT"://_____[
				...
				if(FW_V2_funcHidden == "") {
					componente.empty().html(unescape(FW_V2_optionList));
			...
			case "AUTO"://_______[
				...
				var pathImg = location.protocol + "//" + location.hostname + 
						FW_V2_IMG_PATH + "/nodes/cinza-cross.gif";
				...
				.blur(function(e)     {
					if($("#hdd_auto_mouseOver_" + pId).val() == "false") {
						FW_V2_retornarTexto(pId, FW_V2_tipoCom);//FW_V2_fecharDivAuto(pId);
					}
				});
			...
			case "DATE"://_______[
				var pathImg = location.protocol + "//" + location.hostname + 
											FW_V2_IMG_PATH + "/icons/calendar.gif";
		...
		if(FW_V2_tipoCom == "TEXT_AREA") {
			FW_V2_DIV_CONTENT_ID.dialog({title: "Editar"}).dialog("open");
			componente.focus();
