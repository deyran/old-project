...
//FW_makeGrid____________[
	var localHostAux = window.location.host;
	var FW_IMAGE_GRID = "http://setetecinfo.com.br/s_r_comercio/images/";
	
	if((localHostAux == "localhost:70") || (localHostAux == "127.0.0.1:70")) {
		FW_IMAGE_GRID = "http://" + localHostAux + "/exodoWeb/images/";
	}
	
	function FW_gridCheck(pId)           	           {
			var checkSelValTxt = "";
			var checkSelVal    = false;
	
			if($(".checkSelVal_" + pId).val() == "false") {
				checkSelVal    = true;
				checkSelValTxt = "true";
				
			} else {
					checkSelVal    = false;
					checkSelValTxt = "false";
					
			}
	
			$(".checkIdGridClass_" + pId).prop("checked", checkSelVal);
			$(".checkSelVal_" + pId).val(checkSelValTxt);
	}
